...
region :: Int -> [Coord] -> Int
region limit seeds = length $ filter (< limit) points
    where
        points = map point [(x, y) | x <- [minX..maxX], y <- [minY..maxY]]
        ...
        point p = sum $ map (sDistance p) seeds
        sDistance p s = (distance fst p s) + (distance snd p s)
